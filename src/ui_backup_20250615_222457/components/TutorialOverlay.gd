...
func _apply_theme() -> void:
	...
	# Style panel with M3 design
	if tooltip_panel:
		M3ComponentApplicator.apply_m3_panel_styling(tooltip_panel, M3ComponentApplicator.PanelVariant.SURFACE)
		
		# Add elevation shadow
		var panel_style = tooltip_panel.get_theme_stylebox("panel")
		if panel_style and panel_style is StyleBoxFlat:
			panel_style.shadow_size = M3DesignTokens.M3_ELEVATION["modal"]
			panel_style.shadow_color = M3DesignTokens.get_color("shadow")
			panel_style.shadow_offset = Vector2(0, 4)
	
	# Style title
	if title_label:
		M3ComponentApplicator.apply_m3_text_styling(title_label, M3ComponentApplicator.TypographyScale.HEADLINE_SMALL)
		title_label.add_theme_color_override("font_color", M3DesignTokens.get_color("primary"))
	
	# Style description
	if description_label:
		description_label.add_theme_color_override("default_color", M3DesignTokens.get_color("on_surface"))
		description_label.add_theme_font_size_override("normal_font_size", M3DesignTokens.M3_TYPE_SCALE["body_large"]["size"])
		description_label.add_theme_font_size_override("bold_font_size", M3DesignTokens.M3_TYPE_SCALE["label_large"]["size"])
	...
	# Style buttons
	if next_button:
		M3ComponentApplicator.apply_m3_button_styling(next_button, M3ComponentApplicator.ButtonVariant.PRIMARY)
	if skip_button:
		M3ComponentApplicator.apply_m3_button_styling(skip_button, M3ComponentApplicator.ButtonVariant.TERTIARY)
	if previous_button:
		M3ComponentApplicator.apply_m3_button_styling(previous_button, M3ComponentApplicator.ButtonVariant.SECONDARY)
	if close_button:
...
func _update_highlight_mask(rect: Rect2) -> void:
	...
	highlight_border.border_color = M3DesignTokens.get_color("primary")
...
func _add_highlight_pulse() -> void:
	...
	var original_color = M3DesignTokens.get_color("primary")
