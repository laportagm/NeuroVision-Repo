...
# Preload shaders
const GLASS_MORPHISM_SHADER = preload("res://src/ui_atomic/effects/shaders/glass_morphism_ui.gdshader")
# Theme transition shader temporarily disabled - file missing
...
func animate_theme_transition(from_theme: Theme, to_theme: Theme, duration: float = 0.3, transition_type: int = 0) -> void:
	...
	# Simple fade transition without shader (shader file missing)
	# TODO: Restore shader-based transition when theme_transition.gdshader is available
	...
	# Simple color fade transition
	transition_overlay.modulate = Color.WHITE
	transition_overlay.color = from_color

	# Animate transition with simple fade
	tween.tween_property(transition_overlay, "color", to_color, duration * 0.5)
	tween.parallel().tween_property(transition_overlay, "modulate:a", 0.0, duration)
