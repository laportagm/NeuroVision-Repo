...
# === PHASE 1 ENHANCEMENT: PROFESSIONAL MEDICAL GRADIENTS ===
...
# === Phase 6: PERFORMANCE-AWARE DESIGN TOKENS ===

# Performance quality level tokens for dynamic adaptation
const M3_PERFORMANCE_TOKENS = {
	"maximum": {
		"blur_radius": 15.0,
		...
		"texture_quality": 1.0,
		"lighting_complexity": 3,  # Full three-point lighting
		"aa_samples": 4
	},
	"high": {
		"blur_radius": 10.0,
		"shadow_intensity": 0.8,
		"animation_duration_multiplier": 0.9,
		"transition_easing": "ease_out",
		"particle_density": 0.75,
		"texture_quality": 0.9,
		"lighting_complexity": 2,  # Two-point lighting
		"aa_samples": 2
	},
	"medium": {
		"blur_radius": 6.0,
		...
		"texture_quality": 0.8,
		"lighting_complexity": 1,  # Single key light
		"aa_samples": 1
	},
	"low": {
		"blur_radius": 3.0,
		"shadow_intensity": 0.3,
		"animation_duration_multiplier": 0.6,
		"transition_easing": "linear",
		"particle_density": 0.25,
		"texture_quality": 0.7,
		"lighting_complexity": 0,  # Minimal lighting
		"aa_samples": 0
	}
}

# Frame rate threshold tokens for adaptive quality
const M3_PERFORMANCE_THRESHOLDS = {
	"target_fps": 60.0,
	"warning_fps": 45.0,
	"critical_fps": 30.0,
	"excellent_fps": 90.0,
	"frame_time_target_ms": 16.67,  # 60 FPS
	"frame_time_warning_ms": 22.22,  # 45 FPS
	"frame_time_critical_ms": 33.33  # 30 FPS
}

# Memory usage tokens for medical-grade stability
const M3_MEMORY_TOKENS = {
	"texture_memory_limit_mb": 200.0,
	"ui_memory_limit_mb": 100.0,
	"total_memory_warning_mb": 500.0,
	"total_memory_critical_mb": 750.0,
	"memory_cleanup_threshold": 0.85,  # 85% of limit
	"memory_optimization_enabled": true
}

# Hardware-specific adaptation tokens
const M3_HARDWARE_TOKENS = {
	"intel_uhd_620": {
...
# Educational platform specific performance tokens
const M3_EDUCATIONAL_PERFORMANCE = {
	"interaction_response_target_ms": 50.0,  # Medical education requirement
	"ui_transition_max_ms": 300.0,  # Smooth but not distracting
	"structure_selection_delay_ms": 100.0,  # Visual feedback delay
	"performance_monitoring_interval_ms": 500.0,  # Monitoring frequency
	"quality_adaptation_cooldown_s": 3.0,  # Prevent rapid quality changes
	"educational_fps_minimum": 30.0,  # Minimum for effective learning
	"accessibility_compliance_level": "WCAG_AAA"
}

# Material 3 performance-aware color tokens
const M3_PERFORMANCE_COLORS = {
	"performance_excellent": Color(0.2, 0.8, 0.2, 1.0),  # Green for 60+ FPS
	"performance_good": Color(0.8, 0.8, 0.2, 1.0),       # Yellow for 45-60 FPS
	"performance_warning": Color(0.8, 0.5, 0.2, 1.0),    # Orange for 30-45 FPS
	"performance_critical": Color(0.8, 0.2, 0.2, 1.0),   # Red for <30 FPS
	"memory_normal": Color(0.2, 0.6, 0.8, 1.0),          # Blue for normal memory
	"memory_warning": Color(0.8, 0.6, 0.2, 1.0),         # Orange for high memory
	"memory_critical": Color(0.8, 0.2, 0.4, 1.0),        # Red for critical memory
	"quality_maximum": Color(0.5, 0.2, 0.8, 1.0),        # Purple for maximum quality
	"quality_automatic": Color(0.2, 0.7, 0.5, 1.0)       # Teal for auto adaptation
}

# Performance-aware animation tokens
const M3_PERFORMANCE_ANIMATIONS = {
	"maximum": {
		"glass_transition_duration": 400.0,
		"camera_transition_duration": 1500.0,
		"ui_fade_duration": 200.0,
		"structure_highlight_duration": 300.0,
		"easing_curve": "ease_out_cubic"
	},
	"high": {
		"glass_transition_duration": 300.0,
		"camera_transition_duration": 1200.0,
		"ui_fade_duration": 150.0,
		"structure_highlight_duration": 250.0,
		"easing_curve": "ease_out"
	},
	"medium": {
		"glass_transition_duration": 200.0,
		"camera_transition_duration": 1000.0,
		"ui_fade_duration": 100.0,
		"structure_highlight_duration": 200.0,
		"easing_curve": "ease_in_out"
	},
	"low": {
		"glass_transition_duration": 100.0,
		"camera_transition_duration": 600.0,
		"ui_fade_duration": 50.0,
		"structure_highlight_duration": 100.0,
		"easing_curve": "linear"
	}
}

# Helper functions for performance-aware design

static func get_performance_token(quality_level: String, token_name: String) -> Variant:
	"""Get performance token for specific quality level"""
	if not M3_PERFORMANCE_TOKENS.has(quality_level):
		push_warning("Unknown quality level: " + quality_level)
		return M3_PERFORMANCE_TOKENS["medium"].get(token_name, null)
	
	return M3_PERFORMANCE_TOKENS[quality_level].get(token_name, null)

static func get_hardware_profile(gpu_type: String) -> Dictionary:
	"""Get hardware-specific performance profile"""
	return M3_HARDWARE_TOKENS.get(gpu_type, M3_HARDWARE_TOKENS["integrated_graphics"])

static func get_performance_color(metric_type: String, value: float) -> Color:
	"""Get performance indicator color based on metric"""
	match metric_type:
		"fps":
			if value >= M3_PERFORMANCE_THRESHOLDS.target_fps:
				return M3_PERFORMANCE_COLORS.performance_excellent
			elif value >= M3_PERFORMANCE_THRESHOLDS.warning_fps:
				return M3_PERFORMANCE_COLORS.performance_good
			elif value >= M3_PERFORMANCE_THRESHOLDS.critical_fps:
				return M3_PERFORMANCE_COLORS.performance_warning
			else:
				return M3_PERFORMANCE_COLORS.performance_critical
		"memory":
			if value < M3_MEMORY_TOKENS.total_memory_warning_mb:
				return M3_PERFORMANCE_COLORS.memory_normal
			elif value < M3_MEMORY_TOKENS.total_memory_critical_mb:
				return M3_PERFORMANCE_COLORS.memory_warning
			else:
				return M3_PERFORMANCE_COLORS.memory_critical
		_:
			return M3_COLORS.on_surface

static func get_animation_duration(quality_level: String, animation_type: String) -> float:
	"""Get animation duration for quality level"""
	if not M3_PERFORMANCE_ANIMATIONS.has(quality_level):
		quality_level = "medium"
	
	var animation_tokens = M3_PERFORMANCE_ANIMATIONS[quality_level]
	return animation_tokens.get(animation_type, 200.0)

static func should_enable_effect(quality_level: String, effect_name: String) -> bool:
	"""Check if effect should be enabled for quality level"""
	var quality_index = ["low", "medium", "high", "maximum"].find(quality_level)
	if quality_index == -1:
		quality_index = 1  # Default to medium
	
	match effect_name:
		"glass_morphism":
			return quality_index >= 1  # Medium and above
		"shadows":
			return quality_index >= 0  # All levels (but different quality)
		"particle_effects":
			return quality_index >= 2  # High and maximum
		"chromatic_aberration":
			return quality_index >= 3  # Maximum only
		"advanced_lighting":
			return quality_index >= 2  # High and maximum
		_:
			return true

static func get_medical_compliance_requirements() -> Dictionary:
	"""Get medical education platform compliance requirements"""
	return {
		"minimum_fps": M3_EDUCATIONAL_PERFORMANCE.educational_fps_minimum,
		"maximum_memory_mb": M3_MEMORY_TOKENS.total_memory_warning_mb,
		"interaction_response_ms": M3_EDUCATIONAL_PERFORMANCE.interaction_response_target_ms,
		"accessibility_level": M3_EDUCATIONAL_PERFORMANCE.accessibility_compliance_level,
		"visual_stability_required": true,
		"color_accuracy_required": true,
