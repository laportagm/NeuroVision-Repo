@tool
## Direct Scene Optimization Tool for EnhancedExploration
## 
## This tool script directly modifies the EnhancedExplorationScene.tscn file
## to apply all Priority 1 optimizations from the node audit.
## 
## Usage: Run this script in the Godot editor or via command line

extends EditorScript

const SCENE_PATH = "res://scenes/3d/EnhancedExplorationScene.tscn"
const BACKUP_PATH = "res://scenes/3d/EnhancedExplorationScene_backup.tscn"
...
func _run() -> void:
	...
	if not _create_backup():
		push_error("Failed to create backup. Aborting optimization.")
		return
	...
	# 5. Update shader parameters
	changes_made += _optimize_shaders(root)
	...
	if changes_made > 0:
		...
		if error == OK:
			print("\n✅ SUCCESS: Scene optimized and saved!")
			print("   Original backed up to: " + BACKUP_PATH)
			_print_verification_steps()
		else:
			push_error("Failed to save optimized scene: " + str(error))
	else:
		print("\n⚠️  No changes were needed or possible.")
...
func _create_backup() -> bool:
	...
	if dir:
		var error = dir.copy(SCENE_PATH, BACKUP_PATH)
		if error == OK:
			print("✓ Backup created: " + BACKUP_PATH)
			return true
		else:
			push_error("Backup failed: " + str(error))
	return false
...
func _add_missing_nodes(root: Node) -> int:
	...
	if model_holder:
		if not model_holder.has_node("BrainModelLoader"):
			...
			print("  - BrainModelLoader already exists")
	...
	if interaction_system:
		if not interaction_system.has_node("InteractionController"):
			...
			controller.set_script(load("res://src/systems/3d_interaction/BrainInteractionController.gd"))
			interaction_system.add_child(controller)
			controller.owner = root
			print("  ✓ Added InteractionController with script")
			changes += 1
		else:
			print("  - InteractionController already exists")
	...
	if systems_container:
		if not systems_container.has_node("AccessibilityManager"):
			...
			print("  - AccessibilityManager already exists")
	...
	if not root.has_node("EducationalAudioSystem"):
		...
		audio_system.bus = "Master"  # Use Master if Voice bus doesn't exist
		...
		print("  - EducationalAudioSystem already exists")
...
func _optimize_shaders(root: Node) -> int:
	print("\n[5] Optimizing Shader Parameters...")
	var changes = 0
	
	# Find all panels with glass shader
	var panels = [
		root.find_child("EducationalTopBar", true, false),
		root.find_child("AnatomicalInfoPanel", true, false),
		root.find_child("EducationalQuizOverlay", true, false),
		root.find_child("EducationalHelpOverlay", true, false)
	]
		...
		if panel and panel.material:
			...
			if mat and mat.get_shader_parameter("blur_amount") != null:
				...
				if current_blur > 6.0:
					...
					print("  ✓ Reduced blur on " + panel.name + " from " + str(current_blur) + " to 4.0")
					changes += 1
	
	return changes

func _print_verification_steps() -> void:
	print("\n📋 VERIFICATION STEPS:")
	print("1. Open the scene in Godot editor")
	print("2. Check that KeyLight and RimLight are visible")
	print("3. Verify new nodes exist:")
	print("   - BrainModelLoader under BrainModelHolder")
	print("   - InteractionController under BrainInteractionSystem")
	print("   - AccessibilityManager under EducationalSystemsContainer")
	print("   - EducationalAudioSystem at root")
	print("4. Run the scene (F6) and check for errors")
	print("5. Test performance on Intel UHD 620 if available")
	print("\n💡 TIP: If issues occur, restore from backup: " + BACKUP_PATH)